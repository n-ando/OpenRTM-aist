...
mod_spec = { 
	...
	RTM.RTC_MODULE_DESC:      "Tk Motor component (velocity control)", 
...
class MyComponent(RTM.RtcBase):
	def __init__(self, mgr):
		try:
			...
			self.inport = RTM.TimedFloatSeqIn("vel")
	...
	def rtc_active_do(self):
		try:
			...
			if len(val.data) == 6:
				for i in range(6):
					self.v[i] += val.data[i] / 2
				tkm.set_angle(self.v)
		except:
			print "Exception cought in rtc_active_do()"
