...
/*!
 * @file MyServiceProviderTestComp.cpp
...
{
  MyServiceProviderTestInit(manager);
  MyServiceProviderInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("MyServiceProviderTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("MyServiceProviderTest0");
  if (comp == nullptr)
  {
  ...
  MyServiceProviderTest* testcomp = dynamic_cast<MyServiceProviderTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
