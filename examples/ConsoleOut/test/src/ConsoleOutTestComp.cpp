...
{
  ConsoleOutTestInit(manager);
  ConsoleOutInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("ConsoleOutTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("ConsoleOutTest0");
  if (comp == nullptr)
  {
  ...
  ConsoleOutTest* testcomp = dynamic_cast<ConsoleOutTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
