...
{
  SeqOutTestInit(manager);
  SeqOutInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("SeqOutTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("SeqOutTest0");
  if (comp == nullptr)
  {
  ...
  SeqOutTest* testcomp = dynamic_cast<SeqOutTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
