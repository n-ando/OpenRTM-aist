...
/*!
 * @file MyServiceConsumerTestComp.cpp
...
{
  MyServiceConsumerTestInit(manager);
  MyServiceConsumerInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("MyServiceConsumerTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("MyServiceConsumerTest0");
  if (comp == nullptr)
  {
  ...
  MyServiceConsumerTest* testcomp = dynamic_cast<MyServiceConsumerTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
