...
{
  SeqInTestInit(manager);
  SeqInInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("SeqInTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("SeqInTest0");
  if (comp == nullptr)
  {
  ...
  SeqInTest* testcomp = dynamic_cast<SeqInTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
