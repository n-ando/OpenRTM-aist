 ...
 * @date $Date: 2006-11-10 07:13:44 $
 * @author Shinji Kurihara
 * $Id: SdoConfigurationTests.cpp,v 1.3 2006-11-10 07:13:44 kurihara Exp $
 *
 */
...
public:
   ...
   *    set_device_profile()にてDeviceProfileをセット後,getDeviceProfile()でDeviceProfile
  ...
  void test_set_service_profile() {
    ...
    //=== ServiceProfileListから引数で与えたidを持つServiceProfileの削除を行う===
   ...
   *        get_active_configuration_set(), remove_configuration_set()のテスト
   */
  void test_get_configuration_sets() {
    ...
    // ConfigurationSetが正しくセットされているかを確認するためget_configuration_set()を使用。
    confSet = m_pConf->get_configuration_set("configset_id2");
    setname = "configset_id2";
    getname = confSet->id;
    CPPUNIT_ASSERT(getname == setname);
    ...
    // ConfigurationSetが正しくセットされているかを確認するためget_configuration_sets()を使用。
    confSetList = m_pConf->get_configuration_sets();
    llength = confSetList->length();
    CPPUNIT_ASSERT(llength == 2);
    ...
    // "configset_id2"を要素に持つConfigurationSetが削除されているかを確認するため
